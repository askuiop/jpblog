...
class DaemonOs
{
	...
	public function __construct()
	{
		...
		// Get ancestors
		$ancs = self::_getAncestors($this);
		foreach ($ancs as $i=>$anc) {
			$ancs[$i] = self::_getShortHand($anc);
		}
	...
	public static function factory($force_os = false, $retried = false)
	{
		$Drivers      = array();
		$driversValid = array();
		$class_prefix = "Jims\\MsgQueueBundle\\Service\\OS\\";
		...
		foreach (glob($driver_dir . "/*.php") as $driver_path) {
			...
			if ($driver == "DaemonOSException" || !is_file($driver_path)) {
				continue;
			}
	...
	public function getDataDir()
	{
		...
		if (!$dir) {
			$this->errors[] = 'No data dir found in either in : '.$try_dir;
			return false;
		}
	...
	protected function _testAutoRunProperties($properties = false)
	{
		...
		if (preg_match("@\.php$@", $daemon_filepath) && !file_exists($daemon_filepath)) {
		...
		if (preg_match("@\.php$@", $daemon_filepath) && !is_executable($daemon_filepath)) {
	...
	{
		$weights = array_map(
			array(self::class, "_getAncestorCount"),
			$classes
		);
		arsort($weights);
	...
	/**
	 * Extracts last part of a classname. e.g. self_Ubuntu -> Ubuntu
	 *
	...
	{
		return count(self::_getAncestors($class));
	}
